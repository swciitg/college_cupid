...
class CrushesService {
  ...
  Future<bool> removeCrush(String sharedSecret, String email) async {
    final status = await crushesRepository.removeCrush(sharedSecret);
    if (status) {
      await OneDriveRepository.removeCrush(email);
